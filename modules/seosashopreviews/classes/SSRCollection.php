 ...
 * @since 1.5.0
 */
class SSRCollection implements Iterator, ArrayAccess, Countable
{
	const LEFT_JOIN = 1;
	const INNER_JOIN = 2;
	...
	public function where($field, $operator, $value, $method = 'where')
	{
		...
		if (is_array($value))
		{
			switch (Tools::strtolower($operator))
			{
				case '=' :
				case 'in' :
