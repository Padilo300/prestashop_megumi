...
class ToolsModuleSK
{
	...
	public static function registerSmartyFunctions()
	{
		...
		if (!array_key_exists('getValue', $smarty->registered_plugins['function']))
			smartyRegisterFunction($smarty, 'function', 'getValue', array(__CLASS__, 'getValue'));
		if (class_exists('TransModSK'))
		{
			if (!array_key_exists('ld', $smarty->registered_plugins['modifier']))
				smartyRegisterFunction($smarty, 'modifier', 'ld', array(TransModSK::getInstance(), 'ld'));
		}
	}
	...
	public static function getImageLang($smarty)
	{
		...
		else
		{
			...
			$cookie->id_lang = Context::getContext()->language->id;
		}

		$path = $smarty['path'];
		$module_path = ToolsModuleSK::getModNameForPath(__FILE__).'/views/img/';
	...
	public static function getValue($smarty)
	{
		if (!array_key_exists('name', $smarty))
			throw new Exception('Required argument name in "getValue" smarty function');

		return Tools::getValue($smarty['name']);
	}
		...
		if (function_exists('error_get_last'))
		{
			...
			if ($error && $error['type'] === E_ERROR)
			{
				...
				if (preg_match($memory_regex, $message, $matches))
				{
					...
					$message .= ')';
					LoggerSK::getInstance()->error($message);
					LoggerSK::getInstance()->error(
				...
				die(Tools::jsonEncode(array(
					'hasError' => LoggerSK::getInstance()->hasError(),
					'log' => LoggerSK::getInstance()->getMessages()
				)));
			}
		}
		exit;
	...
	public static function createAjaxApiCall($class)
	{
		...
		if (method_exists($class, $call_method))
		{
			...
			{
				LoggerSK::getInstance()->exception($e);
				die(Tools::jsonEncode(array(
					'hasError' => LoggerSK::getInstance()->hasError(),
					'log' => LoggerSK::getInstance()->getMessages()
				)));
