...
class CDEKNewOrders extends CDEKDocument
{
    ...
    public function __construct(Order $order, $prepayment = false)
    {
        $this->order = $order;
        ...
        foreach ($cart->getProducts() as $index => $product)
        {
            ...
            if ($product['weight'] == 0) {
                $product['weight'] = CDEKCalculate::getProductDimensions(
                    $product,
                    'weight'
                ) * (float)ConfSK::getConf('weight_unit') * 1000;
            ...
            $order['Schedule'] = array(
                'Attempt' => array(
                    '@attributes' => array(
                        ...
                        'TimeBeg' => $order_info->delivery_time_begin.':00:00',
                        'TimeEnd' => $order_info->delivery_time_end.':00:00'
                    )
                )
            );
        }
